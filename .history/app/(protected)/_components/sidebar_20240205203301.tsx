        ...
        <div className='flex flex-col space-y-8'>
            ...
            <div className='flex flex-col gap-2'>
                {sidebarLinks.map((link) => (
                    <Link key={link.path} href={link.path} className={cn('flex gap-3 text-sm items-center hover:bg-neutral-100/10 p-3 rounded-lg', path === link.path ? "bg-neutral-500/20 dark:bg-neutral-100/10" : "")}>
                        {link.icon}
                        {link.name}
                    </Link>
                ))}
